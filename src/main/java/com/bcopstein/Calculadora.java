...
public class Calculadora {
	...
	public static ElementoDeExpressao avalia(String expression) {
        ...
        if (tokens.length < 3){
            ...
            ElementoDeExpressao opEsq = new ValorInteiro(tokens[0]);
            String operador = tokens[1];
            ElementoDeExpressao opDir = new ValorInteiro(tokens[2]);
        ...
        while(pos <= tokens.length - 2){
            ...
            pos++;
            ElementoDeExpressao opDir = new ValorInteiro(tokens[pos]);
            expressao = Calculadora.montaOperadorBinario(opEsq, operador, opDir);
        }
        return  expressao;
    }
    ...
    public static ElementoDeExpressao montaOperadorBinario(ElementoDeExpressao opEsq,String operador,ElementoDeExpressao opDir){
        switch(operador){
            ...
            case "*":
            return new OperadorMultiplicacao(opEsq, opDir);
            case "/":
            return new OperadorDivisao(opEsq, opDir);
            default:
            return null;
        }
    }
